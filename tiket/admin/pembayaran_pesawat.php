...
<div class="navbar-default col-md-2 side" role="navigation">
  ...
  $result = mysql_query("SELECT * FROM konfirmasi_bayar_pesawat INNER JOIN booking_pesawat USING(id_booking_p)");
  $total = mysql_num_rows($result);
  $pages = ceil($total/$halaman); 
      $tampil="select * from konfirmasi_bayar_pesawat INNER JOIN booking_pesawat USING(id_booking_p) order by id_k_bayar desc LIMIT $mulai, $halaman"; 
...
<h3 class="judul">Data Pembayaran Pesawat</h3>
...
<div class="container-fluid">
  ...
  <?php  
	...
	<?php
     ...
     ?> 
      <tr>
        <td class="ukuran2"><?php echo $dataTampil['id_booking_p']; ?></td>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
		...
		if($dataTampil['status']=='lunas'){
			...
			<a data-toggle="tooltip" title="hapus" href="hapus_k_b_pesawat.php?id=<?php echo $dataTampil['id_k_bayar']; ?>&status=<?php $dataTampil['status']; ?>" onclick="return confirm('apakah anda yakin ingin menghapus pesan tiket kereta id <?php echo $dataTampil['id_booking_p'];?>')"><span style="color:red;"class="glyphicon glyphicon-trash"></span></center></td>
			<?php
		}
		else{
			?>
			<a data-toggle="modal" data-target="#myModal"><i class="glyphicon glyphicon-eye-open"></i></a> | 
		<a data-toggle="tooltip" title="konfirmasi" href="konfirmasi_pesan_pesawat.php?id=<?php echo $dataTampil['id_booking_p']; ?>&id2=<?php echo $dataTampil['id_pesawat'];?>&jumlah_p_d=<?php echo $dataTampil['jumlah_p_d']; ?>&jumlah_p_b=<?php echo $dataTampil['jumlah_p_b'] ?>&tgl_pergi=<?php echo $dataTampil['tgl_pergi']; ?>"><span style="color:blue;" class="glyphicon glyphicon-edit"></span></a> 
		| <a data-toggle="tooltip" title="hapus" href="hapus_k_b_pesawat.php?id=<?php echo $dataTampil['id_k_bayar']; ?> &id3=<?php echo $dataTampil['id_booking_p'];?>&status=<?php $dataTampil['status']; ?>" onclick="return confirm('apakah anda yakin ingin menghapus keterangan bayar kereta id <?php echo $dataTampil['id_k_bayar'];?>')"><span style="color:red;"class="glyphicon glyphicon-trash"></span></center></td>
      <?php
		}
		?>
	  </tr>
